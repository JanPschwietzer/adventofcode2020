...
class Berechnung: # german for calculation


#
# This function extracts the minimum amount of the same letter in the password
# We check if it is 1 or 2 digits long and then extract the right amount
    def first_counter():
        for i in range(len(text)):
            if text[i][1] != "-":
                check_min.append(text[i][0] + text[i][1])
                check_min[i] = int(check_min[i])
            else:
                check_min.append(text[i][0])
                check_min[i] = int(check_min[i])
        #print(check_min)
    

#
# This function extracts the maximum amount of the same letter in the password
# We check where the second number starts (after the "-") and check afterwards if its 1 or 2 digits long.
    def sec_counter():
        for i in range(len(text)):
            if text[i][1] == "-":
                if text[i][3] == " ":
                    check_max.append(text[i][2])
                    check_max[i] = int(check_max[i])
                else:
                    check_max.append(text[i][2] + text[i][3])
                    check_max[i] = int(check_max[i])
            if text[i][2] == "-":
                if text[i][4] == " ":
                    check_max.append(text[i][3])
                    check_max[i] = int(check_max[i])
                else:
                    check_max.append(text[i][3] + text[i][4])
                    check_max[i] = int(check_max[i])
        #print(check_max)

#
# This function extracts the letter we have to search for
#
    def letter(): # 4-7 kann buchstaben sein // was a note for myself that the index 4-7 could have been the letter
        for i in range(len(text)):
            if text[i][4].isalpha() == True:
                letter.append(text[i][4])
            elif text[i][5].isalpha() == True:
                letter.append(text[i][5])
            elif text[i][6].isalpha() == True:
                letter.append(text[i][6])
            elif text[i][7].isalpha() == True:
                letter.append(text[i][7])
        #print(letter)

#
# All the calculation happens in the next function
# We count how often the letter is in the whole text because it is easier and then substract 1 from the result
    def berechnung():
        pw_counter = 0
        for i in range(len(text)):
            zaehler = text[i].count(letter[i])
            zaehler = zaehler - 1

            if zaehler <= check_max[i]:
                if zaehler >= check_min[i]:
                    pw_counter = pw_counter + 1
        return pw_counter

#
# I added multiple spaces for the "passchecker" function
# Thats because I did not want to get out of index while searching.
    def line_multi():
        for i in range(len(text)):
            text[i] = text[i] + "                             "

#
# This algorithm checks if the searched letter of the password is on 1 of the positions like stated in the task.
# Probably the algorithm could be shorter but it does it's job so I did not really care.
    def pass_checker():
        valid = 0
        for i in range(len(text)):

            if text[i][7].isalpha() == True:
                if text[i][7 + check_min[i] - 1] == letter[i] and text[i][7 + check_max[i] - 1] != letter[i]:  
                    valid = valid + 1
                if text[i][7 + check_min[i] - 1] != letter[i] and text[i][7 + check_max[i] - 1] == letter[i]:  
                    valid = valid + 1

            elif text[i][7].isspace() == True:

                if text[i][8].isalpha() == True:
                    if text[i][8 + check_min[i] - 1] == letter[i] and text[i][8 + check_max[i] - 1] != letter[i]:  
                        valid = valid + 1
                    if text[i][8 + check_min[i] - 1] != letter[i] and text[i][8 + check_max[i] - 1] == letter[i]:  
                        valid = valid + 1

            elif text[i][8].isspace() == True:

                if text[i][9].isalpha() == True:
                    if text[i][9 + check_min[i] - 1] == letter[i] and text[i][9 + check_max[i] - 1] != letter[i]:  
                        valid = valid + 1
                    if text[i][9 + check_min[i] - 1] != letter[i] and text[i][9 + check_max[i] - 1] == letter[i]:  
                        valid = valid + 1

            elif text[i][9].isspace() == True:

                if text[i][10].isalpha() == True:
                    if text[i][10 + check_min[i] - 1] == letter[i] and text[i][10 + check_max[i] - 1] != letter[i]:  
                        print(text[i][10 + check_min[i] - 1], "=", letter[i])
                        print(text[i][10 + check_max[i] - 1], "!=", letter[i])
                        valid = valid + 1
                    if text[i][10 + check_min[i] - 1] != letter[i] and text[i][10 + check_max[i] - 1] == letter[i]:  
                        valid = valid + 1
        return valid
...
if __name__ == "__main__":
    Berechnung.first_counter()
    Berechnung.sec_counter()
    Berechnung.letter()
    print("Star 1: ", Berechnung.berechnung())
    Berechnung.line_multi()
    print("Star 2: ", Berechnung.pass_checker())
